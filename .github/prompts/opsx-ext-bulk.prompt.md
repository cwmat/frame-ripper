...
Invoke the `openspec-ext-bulk-migrate` skill at `.github/skills/openspec-ext-bulk-migrate/SKILL.md` and follow its workflow exactly. Pay particular attention to the **confirm-the-queue** preflight — never start implementing without showing the user the planned order first.
